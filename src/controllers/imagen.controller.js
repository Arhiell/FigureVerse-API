...
const { imagenSchema } = require('../validations/imagen.validate');
...
exports.crearImagen = async (req, res, next) => {
  try {
    ...
    if (value.id_variante) {
      const [variante] = await pool.query('SELECT id_variante FROM variantes_producto WHERE id_variante = ?', [value.id_variante]);
      if (!variante.length) return res.status(404).json({ message: 'Variante no encontrada' });
    }

    const [result] = await pool.query('INSERT INTO imagenes_productos SET ?', [value]);
...
exports.obtenerImagen = async (req, res, next) => {
  try {
    const [rows] = await pool.query('SELECT * FROM imagenes_productos WHERE id_imagen = ?', [req.params.id]);
...
exports.actualizarImagen = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query('UPDATE imagenes_productos SET ? WHERE id_imagen = ?', [value, req.params.id]);
...
exports.eliminarImagen = async (req, res, next) => {
  try {
    const [result] = await pool.query('DELETE FROM imagenes_productos WHERE id_imagen = ?', [req.params.id]);
    if (result.affectedRows === 0) return res.status(404).json({ message: 'Imagen no encontrada' });
