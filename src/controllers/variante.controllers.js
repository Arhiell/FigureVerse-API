...
const {
  varianteSchema,
} = require("../middlewares/validators/validate_variante");
...
exports.crearVariante = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query("INSERT INTO variantes_producto SET ?", [
      value,
    ]);
...
exports.obtenerVariante = async (req, res, next) => {
  try {
    const [rows] = await pool.query(
      "SELECT * FROM variantes_producto WHERE id_variante = ?",
      [req.params.id]
    );
    if (!rows.length)
...
exports.actualizarVariante = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(
      "UPDATE variantes_producto SET ? WHERE id_variante = ?",
      [value, req.params.id]
    );
